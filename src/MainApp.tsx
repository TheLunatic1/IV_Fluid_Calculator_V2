...
export default function MainApp() {
  ...
  const calculate = () => {
    ...
    if (!vol || vol <= 0) {
      Alert.alert("Error", "Please enter valid fluid volume");
      return;
    }
    if (hrs === 0 && mins === 0) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      {activeTab === 'calculator' ? (
          ...
          <LinearGradient colors={theme === 'dark' ? ['#0f172a', '#1e2937'] : ['#f8fafc', '#e0f2fe']} style={{ flex: 1, padding: 20 }}>
            ...
            <View style={{ backgroundColor: colors.card, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: colors.border }}>
              ...
              <View style={{ marginBottom: 20 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 12 }}>
                  <TouchableOpacity 
                    onPress={() => setDropFactor(15)}
                    style={{ flex: 1, padding: 16, backgroundColor: dropFactor === 15 ? colors.primary : colors.card, borderRadius: 12, alignItems: 'center', borderWidth: 2, borderColor: dropFactor === 15 ? colors.primary : colors.border }}
                  >
                  ...
                  <TouchableOpacity 
                    ...
                    style={{ flex: 1, padding: 16, backgroundColor: dropFactor === 60 ? colors.primary : colors.card, borderRadius: 12, alignItems: 'center', borderWidth: 2, borderColor: dropFactor === 60 ? colors.primary : colors.border }}
                  >
            ...
            {result !== null && (
              <View style={{ marginTop: 24, backgroundColor: colors.card, borderRadius: 20, padding: 24, borderWidth: 1, borderColor: colors.border }}>
                ...
                {mlPerHour && <Text style={{ textAlign: 'center', color: colors.accent, fontWeight: '600' }}>≈ {mlPerHour} ml/hour</Text>}
              </View>
            )}
          </LinearGradient>
        </ScrollView>
        ...
        <ScrollView style={{ flex: 1, padding: 20, backgroundColor: colors.background }}>
          <LinearGradient colors={theme === 'dark' ? ['#0f172a', '#1e2937'] : ['#f8fafc', '#e0f2fe']} style={{ flex: 1, borderRadius: 20, padding: 24 }}>
            ...
            <TouchableOpacity 
              onPress={toggleTheme} 
              style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.card, padding: 20, borderRadius: 16, marginBottom: 20 }}
            >
      ...
      {/* Bottom Tab Bar - Now Theme Aware */}
      <View style={{ 
        flexDirection: 'row', 
